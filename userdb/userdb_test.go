...
func Test_UserDB_File(t *testing.T) {
	...
	err = udb1.InsertUser("james", "jamess-secret")
	if err != nil {
		t.Errorf("Fail: %v", err)
	}
	err = udb1.DeleteUser("james")
	if err != nil {
		t.Errorf("Fail: %v", err)
	}
	...
	exists, _ := udb2.UserExists("james")
	if exists {
		t.Errorf("oh no : %v", err)
	}
	...
	lines, err := util.ReadLines(udb1.fileName)
	if err != nil {
		t.Errorf("Fail: %v", err)
	}
	...
	_, err = util.ReadLines("test_files/userdb_test_file_does_not_exist")
	if err == nil {
